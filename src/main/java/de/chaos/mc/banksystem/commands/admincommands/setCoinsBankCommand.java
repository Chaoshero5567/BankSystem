...
public class setCoinsBankCommand implements CommandExecutor {
    ICoinsInterface coinsInterface;

    public setCoinsBankCommand(ICoinsInterface coinsInterface) {
            ...
            if (player.hasPermission("Coins.setCoins")) {
                if (args.length == 2) {
                    if (Bukkit.getPlayer(args[0])!= null) {
                        ...
                        if (amount != null) {
                            coinsInterface.setCoinsBank(target.getUniqueId(), amount);
                        } else {
